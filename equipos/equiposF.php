<!DOCTYPE html>
<html lang="es">

<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <link rel="icon" href="img/d-uem.png">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css"
    integrity="sha512-KfkfwYDsLkIlwQp6LFnl8zNdLGxu9YAA1QvwINks4PhcElQSvqcyVLLD9aMhXd13uQjoXtEKNosOWaZqXgel0g=="
    crossorigin="anonymous" referrerpolicy="no-referrer" />
  <link rel="stylesheet" href="../main/style.css">
  <title>Futbol Sala UEM</title>
</head>

<body>
  <?php require '../main/header.php'?>
  
   <!-- CLASIFICACION -->
   <section class="seccionE">
    <div class="contenedor_resultados_clasificacion">
      <div class="top_div_equipos">
        <div class="h2_top_div">
          <h2 class="h2_table_top">Fútbol Sala</h2>
        </div>
        <div class="titulo_top_div">
          <img src="../img/UEM-futbol.jpeg" id="image_equipo">
        </div>
        <div class="introduccion_equipo">
          <p>Nuestro equipo de fútbol sala cuenta con 11 jugadores y un entrenador. Este es el primer año que juegan juntos, aunque el equipo cuenta con 2 jugadores más veteranos. Entrenan y juegan sus partidos en el polideportivo del edificio D, ¡Vente a animarles!</p>
        </div>
      </div>
      <div class="grid-top">
        <h2 class="h2_table">Resultados</h2>
        <table class="resultados">
          <tr>
            <th class="jornada"></th>
            <th class="uni">Equipo local</th>
            <th class="resultado_partido">Resultado</th>
            <th class="uni">Equipo visitante</th>
          </tr>
          <tr>
            <th class="jornada">Jornada 1</th>
            <td class="uni">Univ. Francisco de Vitoria</td>
            <th class="resultado_partido">4-5</th>
            <td class="uni">Univ. Europea</td>
          </tr>
          <tr>
            <th class="jornada">Jornada 2</th>
            <td class="uni">Univ. Europea</td>
            <th class="resultado_partido">6-8</th>
            <td class="uni">Univ. Politécnica</td>
          </tr>
          <tr>
            <th class="jornada">Jornada 3</th>
            <td class="uni">Univ. Alcalá</td>
            <th class="resultado_partido">8-1</th>
            <td class="uni">Univ. Europea</td>
          </tr>
          <tr>
            <th class="jornada">Jornada 4</th>
            <td class="uni">Univ. Europea</td>
            <th class="resultado_partido">5-2</th>
            <td class="uni">Univ. Nebrija</td>
          </tr>
          <tr>
            <th class="jornada">Jornada 5</th>
            <td class="uni">Univ. Ceu San Pablo</td>
            <th class="resultado_partido">4-11</th>
            <td class="uni">Univ. Europea</td>
          </tr>
          <tr>
            <th class="jornada">Jornada 6</th>
            <td class="uni">Univ. Europea</td>
            <th class="resultado_partido">4-2</th>
            <td class="uni">Univ. Rey Juan Carlos</td>
          </tr>
          <tr>
            <th class="jornada">Jornada 7</th>
            <td class="uni">Univ. Complutense</td>
            <th class="resultado_partido">19:30</th>
            <td class="uni">Univ. Europea</td>
          </tr>
        </table>
      </div>
      
      <div class="grid-bot">
        <h2 class="h2_table">Clasificación</h2>
        <table class="clasificacion_table">
          <tr>
            <th colspan="2" rowspan="2">Equipos</th>
            <th class="puntos" rowspan="2">Puntos</th>
            <th class="partidos" colspan="4">Partidos</th>
            <th class="goles" colspan="2">Goles</th>
          </tr>
          <tr>
            <th class="jgep">Ganados</th>
            <th class="jgep">Empatados</th>
            <th class="jgep">Perdidos</th>
            <th class="jgep">Jugados</th>
            <th class="goles">F</th>
            <th>C</th>
          </tr>
          <tr>
            <th class="clasifNum">1</th>
            <td class="unis">Univ. Alcalá</td>
            <td class="puntos">18</td>
            <td class="jgep">6</td>
            <td class="jgep">0</td>
            <td class="jgep">0</td>
            <td class="jgep">6</td>
            <td class="goles">47</td>
            <td>10</td>
          </tr>
          <tr>
            <th class="clasifNum">2</th>
            <td class="unis">Univ. Complutense</td>
            <td class="puntos">15</td>
            <td class="jgep">5</td>
            <td class="jgep">0</td>
            <td class="jgep">1</td>
            <td class="jgep">6</td>
            <td class="goles">55</td>
            <td>14</td>
          </tr>
          <tr>
            <th class="clasifNum">3</th>
            <td class="unis">Univ. Politécnica</td>
            <td class="puntos">13</td>
            <td class="jgep">4</td>
            <td class="jgep">1</td>
            <td class="jgep">0</td>
            <td class="jgep">5</td>
            <td class="goles">38</td>
            <td>22</td>
          </tr>
          <tr>
            <th class="clasifNum">4</th>
            <td class="unis">Univ. Europea</td>
            <td class="puntos">12</td>
            <td class="jgep">4</td>
            <td class="jgep">0</td>
            <td class="jgep">2</td>
            <td class="jgep">6</td>
            <td class="goles">32</td>
            <td>28</td>
          </tr>
          <tr>
            <th class="clasifNum">5</th>
            <td class="unis">Univ. Rey Juan Carlos</td>
            <td class="puntos">12</td>
            <td class="jgep">4</td>
            <td class="jgep">0</td>
            <td class="jgep">2</td>
            <td class="jgep">6</td>
            <td class="goles">22</td>
            <td>21</td>
          </tr>
          <tr>
            <th>6</th>
            <td class="unis">Univ. Carlos III</td>
            <td class="puntos">10</td>
            <td class="jgep">3</td>
            <td class="jgep">1</td>
            <td class="jgep">2</td>
            <td class="jgep">6</td>
            <td class="goles">25</td>
            <td>20</td>
          </tr>
          <tr>
            <th class="clasifNum">7</th>
            <td class="unis">Univ. Pontífica de Comillas</td>
            <td class="puntos">7</td>
            <td class="jgep">2</td>
            <td class="jgep">1</td>
            <td class="jgep">3</td>
            <td class="jgep">6</td>
            <td class="goles">29</td>
            <td>31</td>
          </tr>
          <tr>
            <th class="clasifNum">8</th>
            <td class="unis">Univ. Nebrija</td>
            <td class="puntos">7</td>
            <td class="jgep">2</td>
            <td class="jgep">1</td>
            <td class="jgep">3</td>
            <td class="jgep">6</td>
            <td class="goles">25</td>
            <td>32</td>
          </tr>
          <tr>
            <th class="clasifNum">9</th>
            <td class="unis">Univ. CEU San Pablo</td>
            <td class="puntos">3</td>
            <td class="jgep">1</td>
            <td class="jgep">0</td>
            <td class="jgep">4</td>
            <td class="jgep">5</td>
            <td class="goles">12</td>
            <td>31</td>
          </tr>
          <tr>
            <th class="clasifNum">10</th>
            <td class="unis">Univ. Francisco de Vitoria</td>
            <td class="puntos">3</td>
            <td class="jgep">1</td>
            <td class="jgep">0</td>
            <td class="jgep">5</td>
            <td class="jgep">6</td>
            <td class="goles">24</td>
            <td>43</td>
          </tr>
        </table>
      </div>
    </div>
  </section>
  <script src="../script.js"></script>
</body>
<footer>
  <h1>Ignacio y Samuel, 2022</h1>
</footer>

</html>